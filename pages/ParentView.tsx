...
import { CheckCircle2, Clock, Layers, Sparkles, XCircle, Banknote, AlertCircle, Calendar, Palette, Music, BookOpen, Trophy, Activity, Link, Youtube, FileText, Image, ChevronRight, PlayCircle, ExternalLink } from 'lucide-react';
...
export const ParentView: React.FC<ParentViewProps> = ({ teacherId, studentId }) => {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] font-sans text-slate-800 selection:bg-indigo-100 pb-32">
      ...
      {/* --- HERO SECTION --- */}
      <div className="relative bg-gradient-to-b from-white to-[#F8FAFC] pb-4 pt-8 px-6 rounded-b-[2rem] shadow-sm mb-4 border-b border-slate-100">
        ...
        <div className="flex justify-center mb-6">
            <div className="h-40 w-full max-w-[280px] flex items-center justify-center relative transition-transform hover:scale-105 duration-500">
                {isCustomLogo ? (
                    ...
                    <SchoolIcon size={100} className="text-indigo-600 drop-shadow-lg" strokeWidth={1} />
                )}
            </div>
        </div>

        {/* STUDENT IDENTITY CARD (Glass - Slimmer) */}
        <div className="max-w-sm mx-auto bg-white/60 backdrop-blur-xl border border-white/60 rounded-xl p-3 shadow-lg shadow-indigo-100/50 flex items-center justify-between">
            <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-lg bg-slate-800 text-white flex items-center justify-center text-base font-bold shadow-md">
                ...
                <div>
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5">
                        ...
                        <span className="text-[9px] font-bold text-slate-500 uppercase tracking-wide">Aktif Öğrenci</span>
                    </div>
                </div>
            </div>
        </div>
      ...
      <div className="max-w-md mx-auto px-5 space-y-4">
        ...
        {/* --- NEXT LESSON CARD (Slimmer) --- */}
        <div className="group relative bg-slate-900 rounded-3xl p-5 text-white shadow-xl shadow-slate-200 overflow-hidden cursor-default transition-transform active:scale-[0.99]">
        ...
        {/* --- PAYMENT STATUS (Slimmer) --- */}
        <div className="grid grid-cols-2 gap-3">
            <div className="bg-white p-3 rounded-2xl border border-slate-100 shadow-sm flex flex-col justify-center">
                <p className="text-[8px] font-bold text-slate-400 uppercase tracking-wider mb-1">SON ÖDEME</p>
                <div className="flex items-center gap-2">
                    <div className="w-1.5 h-1.5 rounded-full bg-emerald-500"></div>
                    <span className="text-xs font-bold text-slate-700">{lastPaymentStr}</span>
                </div>
            </div>
            <div className="bg-white p-3 rounded-2xl border border-slate-100 shadow-sm flex flex-col justify-center">
                <p className="text-[8px] font-bold text-indigo-400 uppercase tracking-wider mb-1">GELECEK ÖDEME</p>
                <div className="flex items-center gap-2">
                    <div className="w-1.5 h-1.5 rounded-full bg-indigo-500"></div>
                    <span className="text-xs font-bold text-slate-700">{nextPaymentStr}</span>
                </div>
            </div>
            {student.debtLessonCount > 0 && (
                <div className="col-span-2 bg-indigo-50 p-2.5 rounded-xl border border-indigo-100 flex items-center justify-between px-4">
                    <div className="flex items-center gap-2">
                        <Layers size={14} className="text-indigo-400"/>
                        <span className="text-[10px] font-bold text-indigo-900">Bu Dönem İşlenen</span>
                    </div>
                    <span className="text-xs font-black text-indigo-600 bg-white px-2 py-0.5 rounded-md shadow-sm">{student.debtLessonCount} Ders</span>
                </div>
            )}
        </div>

        {/* --- TIMELINE HISTORY (Tighter) --- */}
        <div>
            <div className="flex items-center justify-between mb-3 px-1">
                <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">DÖNEM HAREKETLERİ</h3>
                <span className="text-[8px] font-bold text-slate-300 bg-slate-50 px-2 py-0.5 rounded-lg">Filtreli</span>
            </div>
            
            <div className="bg-white rounded-[1.5rem] border border-slate-100 shadow-sm p-2">
                {currentPeriodHistory.length === 0 ? (
                    ...
                    <div className="relative pl-5 pt-2 pb-2 space-y-4">
                ...
                <div className="grid grid-cols-1 gap-3">
                    {student.resources.map(res => (
                        <a 
                            ...
                            className="group relative bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden active:scale-[0.98] transition-all hover:shadow-md block"
                        >
                            ...
                            {res.type === 'IMAGE' ? (
                                <div className="aspect-video w-full bg-slate-100 relative overflow-hidden">
                                    ...
                                    <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent flex flex-col justify-end p-3">
                                        <div className="flex items-center gap-1.5 text-white/90 mb-0.5">
                                            <Image size={12} />
                                            <span className="text-[9px] font-bold uppercase tracking-wider">RESİM / NOTA</span>
                                        </div>
                                        <h4 className="text-white font-bold text-xs truncate">{res.title}</h4>
                                    </div>
                                </div>
                            ) : (
                                // If LINK/VIDEO, show icon card
                                <div className="p-3 flex items-center gap-3">
                                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center text-white shadow-md shrink-0 ${
                                        ...
                                        {res.type === 'VIDEO' ? <Youtube size={18} /> : res.type === 'PDF' ? <FileText size={18} /> : <Link size={18} />}
                                    </div>
                                    <div className="min-w-0 flex-1">
                                        <h4 className="font-bold text-slate-800 text-xs truncate">{res.title}</h4>
                                        <div className="flex items-center gap-1 mt-0.5 text-slate-400">
                                            <ExternalLink size={10} />
                                            <span className="text-[9px] font-bold uppercase tracking-wider">{res.type}</span>
                                        </div>
                                    </div>
        ...
        <div className="text-center pt-8 opacity-40">
            <div className="inline-flex items-center gap-1 text-[8px] font-bold text-slate-400 uppercase tracking-widest bg-white/50 px-2 py-0.5 rounded-full border border-slate-100">
